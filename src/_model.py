...
class SpectralJoint:
    ...
    def fit(self, views: list, labels: torch.Tensor = None):
        ...
        multi_spectral_trainer = MultiSpectralTrainer(self.config, self.device)
        self.multi_spectral_net = multi_spectral_trainer.train(
    ...
    def predict(self, views: list) -> np.ndarray:
        ...
        with torch.no_grad():
            embeddings = (
                self.multi_spectral_net(views, is_orthonorm=False)[0]
                .detach()
                .cpu()
                .numpy()
            )
